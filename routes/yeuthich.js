...
// });
router.post('/', (req, res) => {
  const { MaNguoiDung, MaPhim } = req.body;
  db.query('INSERT INTO YeuThich (MaNguoiDung, MaPhim) VALUES (?, ?)', [MaNguoiDung, MaPhim], (err, result) => {
    if (err) {
      res.status(500).json({ error: err.message });
    } else {
      res.status(201).json({ message: 'Phim đã được thêm vào danh sách yêu thích' });
    }
  });
});
router.delete('/:maNguoiDung/:maPhim', (req, res) => {
